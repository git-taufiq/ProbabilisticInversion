...
!      EG(k)=sum(min(Dc(1:nxt,1:nzt-2),slip1(1:nxt,1:nzt-2,k))*peak_xz(1:nxt,1:nzt-2))/2.*dh*dh   ! Dissipated breakdown work (protoze je tam ten min)
      EG(k)=sum(peak_xz(1:nxt,1:nzt-2)*(Dc(1:nxt,1:nzt-2)-(Dc(1:nxt,1:nzt-2)-slip1(1:nxt,1:nzt-2,k))/Dc(1:nxt,1:nzt-2)*max(0.,Dc(1:nxt,1:nzt-2)-slip1(1:nxt,1:nzt-2,k))))/2.*dh*dh   ! Dissipated breakdown work
...
!    CALL meansigma(SEA(:,:,:)/max(1.,T0A(:,:,:)),NM) 
    close(201)
